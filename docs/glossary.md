...
**Version** — The content identity of a resource's spec: the SHA-256 of its canonical serialisation, written as an `@<version>` suffix on a reference. A version is immutable — the same version always denotes the same content — and unordered, so two versions say only that content differs, never which came first. A reference with no suffix means whatever is current when it is resolved; a runtime resolves references once, at task creation, so editing a manifest never changes what an already-running task does. See [References and Versions](./concepts.md#references-and-versions). Unrelated to `status.revision`, which is a change cursor on a task rather than an identity for a manifest.
