...
6. `status.phase` and `status.terminal_reason` follow the lifecycle defined in [Task Context](./task-context.md): an `idle` task awaits its next input, a `processing` task is busy, and `terminal_reason` MUST be `null` for any non-terminal task. A terminal task MUST NOT be mutated further.
7. `status.revision` MUST be an opaque string that changes whenever the status object or the task's [messages](./task-io.md#messages) change. Consumers compare revisions only for inequality — the value carries no ordering or content semantics.
