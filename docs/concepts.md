...
A resource is identified by its name. A name identifies a single agent or tool, and in case of a conflict with an agent and a tool sharing the same name, the tool takes precendence.

A name is lower-case letters, digits and single underscores, starts and ends with a letter or digit, and is at most 63 characters — `[a-z0-9]+(_[a-z0-9]+)*`. The form is narrow on purpose, as it is used verbatim as the function name the LLM sees. `input` and `llm` are additionally reserved names, because each already names part of the task context that a capability of the same name would shadow.

A reference MAY be written with dashes in place of underscores and MUST be read as the same name: `weekly-report` names `weekly_report`. A dash cannot appear in a name, so a reference carrying one names either the underscored resource or nothing at all.

- A tool is referenced as `{tool_name}`.
- An agent is referenced as `{agent_name}`, or equivalently as `agent://{agent_name}`. The qualified form MUST be used to reference an agent where a tool of the same name exists. A runtime MAY additionally interpret `agent://{path}/{agent_name}` as an agent hosted elsewhere.

How a runtime scopes the resources it holds — whether one set is visible to everything it runs, or it partitions them per team, per tenant or otherwise — is not part of this specification. A manifest names what it needs and nothing about where that resource lives, which is what lets the same manifest run unchanged on a runtime that partitions and one that does not.
...
**Connections** are the other half of the pipeline, and a deliberately different shape. Where a parameter is declared in the manifest and filled in per invocation, a connection is a named provider a tool's execution templates read from at the point of use — `{connection('github').base_url}` for non-secret configuration, `{connection('github').service_auth().token}` for a credential. Nothing about a connection is declared in the manifest: the tool names a provider and stops there, and the runtime decides what answers that name. Connections are never exposed to the LLM and never generated by it. They apply only to tools; agents do not name connections.
