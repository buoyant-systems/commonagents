...
10. **`parameters`** — When present, defines the structured input this agent accepts. The schema itself is static — no interpolation. Uses [`ParameterSchema`](../reference/parameters.md) semantics:
    - `required` — a list of names on the parameters object that the caller must supply a value for. A name it does not carry may be omitted, and is null when it is.
    - A property **with** a `default` has it supplied whenever nothing else does, whether or not it is required.
    - `require_binding` — a list of names, a **validation constraint**: the parent agent invoking this sub-agent must supply a binding for each. Without a binding the configuration is invalid. It is the binding that hides the parameter from the LLM.
