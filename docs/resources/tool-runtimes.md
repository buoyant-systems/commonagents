...
A runtime backend is the execution engine behind a tool capability — it determines how a capability invocation is translated into a real-world action, whether that's an HTTP request, a filesystem read, a CEL computation, or an MCP server call.
